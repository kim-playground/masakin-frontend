...
const CreateRecipe = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    cookingTime: "",
    category: "",
    imageUrl: "", // Simplified for now, assuming URL input
    ingredients: [""],
    steps: [""],
    videoUrl: "",
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        steps: cleanSteps,
        images: formData.imageUrl ? [formData.imageUrl] : [], // Array as expected by backend
        videoUrl: formData.videoUrl,
        // Portion and Difficulty could be added
        portion: 2,
        difficulty: "medium",
      };

      await recipesAPI.create(payload);
      navigate("/profile"); // Redirect to profile to see the new recipe
    } catch (err) {
